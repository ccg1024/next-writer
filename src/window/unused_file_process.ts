...
import fs from 'fs';
import { addCache, exitCache, getCache } from './unused_cache';
import { FileDescriptor, FileState, ReadFileDescriptor, RootWorkstationFolderInfo, RootWorkstationInfo } from '_types';
 ...
 */
export async function openFileProcess(win: BrowserWindow, opts?: OpenDialogOptions): Promise<string | null> {
 ...
 */
export async function openMarkdownFileProcess(win: BrowserWindow): Promise<string | null> {
 ...
 */
export async function createFileProcess(win: BrowserWindow): Promise<string | null> {
 ...
 */
export async function handleOpenFile(win: BrowserWindow): Promise<ReadFileDescriptor | null> {
 ...
 * */
export async function constructFileData(filePath: string): Promise<ReadFileDescriptor> {
...
export function generateReadFileIpcValue(filePath: string, content: string, isChange = false): ReadFileDescriptor {
...
export function generateFileDescripter(filePath: string, isChange = false): FileDescriptor {
  return {
    isChange: isChange,
    path: filePath,
...
export async function constructVirtualFileData(filePath: string): Promise<ReadFileDescriptor> {
  addCache(filePath, {
    isChange: false,
    content: ''
  });
...
export function initialRootWorkplatform() {
  ...
  if (!root) {
    throw new Error('The root dir path is empty, did `initialRootWorkplatform` function place wrong way?');
  }
...
export function readRootWorkstationInfo() {
  ...
  if (!root) {
    throw new Error('The root dir path is empty, did `readRootWorkstationInfo` function place right way?');
  }
  if (!fs.existsSync(infoPath)) {
    ...
    fs.promises
      ...
      .then(value => {
        ...
        const stageInfo = JSON.parse(value.toString('utf8')) as RootWorkstationInfo;
...
export async function writeRootWorkstationInfo() {
  ...
  if (!root) {
    throw new Error('The root dir path is empty, did `writeRootWorkstationInfo` function place right way?');
  }
  const stageInfo = global._next_writer_windowConfig.stageWorkplatformInfo;
  fs.promises.writeFile(infoPath, JSON.stringify(stageInfo, null, 2)).catch(err => {
    throw err;
  });
}
...
async function asyncFileTree(root: string, cache: RootWorkstationInfo) {
  ...
  for await (const file of files) {
    ...
    if (stats.isDirectory()) {
      ...
      const description = frontMatter.content.length > 100 ? frontMatter.content.slice(0, 100) : frontMatter.content;
      _files.push({
        name: file,
  ...
  cache.folders = folders.sort((a, b) => new Date(a.birthtime).valueOf() - new Date(b.birthtime).valueOf());
  cache.files = _files.sort((a, b) => new Date(a.birthtime).valueOf() - new Date(b.birthtime).valueOf());
...
export function addLibraryFile(library: string) {
  ...
  const relative = newFilePath.startsWith('.') ? newFilePath : `./${newFilePath}`;
  addWorkstationInfo(global._next_writer_windowConfig.rootWorkplatformInfo, 'file', relative);
  addWorkstationInfo(global._next_writer_windowConfig.stageWorkplatformInfo, 'file', relative);
...
export function addLibrary(library: string) {
  ...
  addWorkstationInfo(global._next_writer_windowConfig.rootWorkplatformInfo, 'folder', library);
  addWorkstationInfo(global._next_writer_windowConfig.stageWorkplatformInfo, 'folder', library);
  writeRootWorkstationInfo().catch(err => {
    throw err;
  });
}
