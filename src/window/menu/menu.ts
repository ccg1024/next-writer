...
import type { MenuItemConstructorOptions, MenuItem, BrowserWindow, KeyboardEvent } from 'electron';
...
import { handleToggleFloatMenu, handleToggleHeadNav, handleToggleMideBar, handleToggleSidebar } from './menu-callback';
import { alertError } from '../unused_dialog';

// menu callback
async function editorTypewriter(menuItem: MenuItem, win: BrowserWindow, _event: KeyboardEvent) {
  if (!win) return;
  ...
  const isShow = menuItem.checked ? !!global._next_writer_windowConfig.renderConfig.focusMode : false;
  ...
  const focusModeMenuItem = editorMenuItem.submenu?.items.find(item => item.label === '专注模式');
...
async function _openFile(_: unknown, win: BrowserWindow) {
  ...
  win.webContents.send(ipcChannel['main-to-render'].editor_component, editorChannelValue);
}
...
async function insertImage(_m: MenuItem, win: BrowserWindow, _e: KeyboardEvent) {
