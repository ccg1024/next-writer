...
import { CONFIG_DIR_NAME, LOG_DIR_NAME, ROOT_DIR_NAME } from 'bin/index.es';
...
/**
 * Initialize the system, invoke the method of this instance to prepare the applicatoin work enviroment
 */
@injectable()
class NextSysInit implements SysInit {
  private _ipcServer: IpcServer;
  private _fileSystem: FileSystem;
  private _mainGlobal: MainGlobal;

  constructor(
    @inject(TYPES.FileSystem) _fileSystem: FileSystem,
    @inject(TYPES.MainGlobal) _mainGlobal: MainGlobal,
    @inject(TYPES.IpcServer) _ipcServer: IpcServer
  ) {
