...
import fs from 'fs';
import { FileState, FrontMatter, RootWorkstationFolderInfo, RootWorkstationInfo } from '_types';
import { writeDefaultConfig } from './unused_file_process';
 ...
 */
export function updateWorkstationInfoWithWrite(libraryFile: string, nextFrontMatter: Partial<FrontMatter>) {
  if (!libraryFile) return;

  const root = global._next_writer_windowConfig.root;
  if (!root) {
    throw new Error('The root is empty from updateWorkstationInfoWithWrite function');
  }
  const relative = libraryFile.startsWith('.') ? libraryFile.substring(2) : libraryFile.substring(root.length + 1);
...
export function recursiveUpdateWorkstationInfoWithWrite(
  ...
  if (pathlist.length === 0) {
    ...
    if (idx === -1)
      throw new Error(`Cannot find file name: ${subpath} in recursiveUpdateWorkstationInfoWithWrite final if`);
    ...
    if (idx === -1) throw new Error(`Cannot find folder name: ${subpath} in recursiveUpdateWorkstationInfoWithWrite`);
    recursiveUpdateWorkstationInfoWithWrite(pathlist, workstation.folders[idx].subfolders, nextFrontMatter);
  }
}
...
export function syncWorkstationInfo(path: string) {
  ...
  for (let i = 0; i < tokens.length; i++) {
    if (i === tokens.length - 1) {
      ...
      stageInfo.files = [
        ...stageInfo.files.sort((a, b) => new Date(a.birthtime).valueOf() - new Date(b.birthtime).valueOf())
      ];
      break;
    }
 ...
 * @param path A relative path, just like './test.md'
 */
export function addWorkstationInfo(workstation: RootWorkstationInfo, type: 'file' | 'folder', path: string) {
  if (!path.startsWith('.')) throw new Error('The path is not start with "."');
  const tokens = path.split('/');
  if (tokens[0] === '.') tokens.shift();
  for (let i = 0; i < tokens.length; i++) {
 ...
 */
export function updateWorkstationInfo(path: string, type: UpdateWorkstationInfoType) {
  if (!path) return;

  const subpath = path.startsWith('./') ? path.substring(2) : path;

  const pathlist = subpath.split('/');
  recursiveUpdateWorkstationInfo(pathlist, global._next_writer_windowConfig.rootWorkplatformInfo, type);
}
function recursiveUpdateWorkstationInfo(
  ...
  if (pathlist.length === 0) {
    ...
    const idx = findFolderName(workstation.folders, subpath);
    if (idx === -1) throw new Error('Cannot find folder name: ' + subpath + ' in recursiveUpdateWorkstationInfo');

    recursiveUpdateWorkstationInfo(pathlist, workstation.folders[idx].subfolders, type);
  }
}
function findFolderName(folders: Array<RootWorkstationFolderInfo>, folder: string) {
  let idx = -1;
  for (let i = 0; i < folders.length; i++) {
 ...
 */
export function deleteRootWorkplatformInfoUnit(pathInLib: string, type: 'file' | 'folder') {
  if (!pathInLib.startsWith('.')) {
    throw new Error('`pathInLib` is not a relatve path in deleteRootWorkplatformInfoUnit function');
  }
  ...
  for (let i = 0; i < tokens.length; i++) {
    if (i === tokens.length - 1) {
      ...
      if (type === 'file') {
        ...
        if (rootFileId == -1) {
          throw new Error('Cannot find root-file in deleteRootWorkplatformInfoUnit');
        }
        const stageFileId = findFileIndex(stageInfo.files, tokens[i]);
        if (stageFileId == -1) {
          throw new Error('Cannot find stage-file in deleteRootWorkplatformInfoUnit');
        }
        ...
        if (rootFolderId == -1) {
          throw new Error('Cannot find root-folder in deleteRootWorkplatformInfoUnit');
        }
        const stageFolderId = findFolderIndex(stageInfo.folders, tokens[i]);
        if (stageFolderId == -1) {
          throw new Error('Cannot find stage-folder in deleteRootWorkplatformInfoUnit');
        }
    ...
    const rootIdx = findFolderIndex(rootInfo.folders, tokens[i]);
    if (rootIdx == -1) {
      throw new Error('Cannot find root-folder name in deleteRootWorkplatformInfoUnit');
    }
    const stageIdx = findFolderIndex(stageInfo.folders, tokens[i]);
    if (stageIdx == -1) {
      throw new Error('Cannot find stage-folder name in deleteRootWorkplatformInfoUnit');
    }
...
export function updateSaveFileMenuItem(filePath: string) {
  ...
  if (!fileMenu) return;
  const saveFileItem = fileMenu.submenu.items.find(item => item.label === '保存');
  if (saveFileItem) {
